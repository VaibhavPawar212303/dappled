...
import { 
  ...
  Quote, Undo, Redo, Code, Heading1, Heading2, Maximize, Minimize, ImageIcon
} from "lucide-react";
...
const Toolbar = ({ 
  ...
  const toggleButton = (isActive: boolean, onClick: () => void, icon: React.ReactNode, label: string) => (
    <button 
      type="button" 
      onClick={onClick} 
      title={label} 
      className={cn(
        ...
        isActive ? "bg-slate-950 text-white" : "bg-transparent hover:bg-slate-200 text-slate-700"
      )}
    >
      {icon}
    </button>
...
export const TiptapEditor: React.FC<Props> = ({ 
  ...
  const toggleExpand = () => {
    ...
    if (controlledOnExpandChange) {
      controlledOnExpandChange(newState);
    } else {
  ...
  const editor = useEditor({
    ...
    extensions: [
      ...
      Image.configure({ 
        inline: true, 
        allowBase64: true, 
        HTMLAttributes: { class: "rounded-md border my-4 max-w-full h-auto" } 
      }),
    ],
    content,
    editorProps: {
      attributes: {
        class: cn(
          ...
          // List padding fixes
          "[&_ol]:pl-5 [&_ul]:pl-5 [&_ol]:list-decimal [&_ul]:list-disc",
          // Expand mode styling
          isExpanded && "min-h-screen border-0 focus-visible:ring-0",
          // Apply custom classes passed from parent
          className
        )
      }
    },
  ...
  // Effect: Update editor content when `content` prop changes externally (e.g. from AI)
  useEffect(() => {
    if (editor && content && content !== editor.getHTML()) {
        // Prevent cursor jumping if user is typing, but allow AI to overwrite
        if (Math.abs(editor.getText().length - (content.length || 0)) > 5) {
  ...
  return (
    <>
      <Dialog open={isImageDialogOpen} onOpenChange={setIsImageDialogOpen}>
        {/* z-[999999] ensures dialog is above full-screen editor */}
        <DialogContent className="sm:max-w-md z-[999999]">
          ...
          <div className="flex items-center justify-center p-4">
            <FileUpload 
              ...
              onChange={(url) => { if (url) onImageUploadComplete(url); }} 
            />
          </div>
        </DialogContent>
      </Dialog>
